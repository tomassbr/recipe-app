...
export function LoginContent() {
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      // Jen přesná cesta bez query — Supabase povoluje redirect URL přesně takto;
      // `?next=...` v redirect_to často způsobí HTTP 400, pokud není v Dashboardu whitelistnuté.
      ...
      // skipBrowserRedirect: ruční přesměrování — v některých prohlížečích / embedded
      // view se automatický assign z GoTrue neprovede spolehlivě.
      ...
      setLocalError(
        "Nepodařilo se získat odkaz na Google. Zkontrolujte v Supabase, že je zapnutý provider Google."
      );
    } catch (e) {
      setLocalError(
        e instanceof Error ? e.message : "Neočekávaná chyba při přihlášení."
      );
    }
    setLoading(false);
  }
      ...
      <GlassCard className="w-full space-y-8 px-8 py-10 md:px-10 md:py-12">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Pokračujte účtem Google pro přístup k PastryCalc.
          </p>
          <p className="mt-3 text-xs text-slate-400">
            Otevírejte app vždy přes stejnou adresu jako v Supabase Redirect URLs
            (např. jen <code className="rounded bg-white/60 px-1">localhost</code>, ne
            mix s <code className="rounded bg-white/60 px-1">127.0.0.1</code>).
          </p>
        </div>
